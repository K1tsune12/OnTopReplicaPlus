...
namespace OnTopReplica.StartupOptions {
    class Factory {
        ...
        private static void LoadSettings(Options options) {
            //Restore the saved opacity (a command line /opacity flag, parsed later, still wins).
            options.Opacity = Settings.Default.Opacity;

            //Only restore position and size once a real (non-zero) size has been stored,
            //so the very first run still opens with a sensible default-sized window.
            var lastSize = Settings.Default.RestoreLastSize;
            if (Settings.Default.RestoreSizeAndPosition && lastSize.Width > 0 && lastSize.Height > 0) {
                options.StartLocation = Settings.Default.RestoreLastPosition;
                options.StartSize = lastSize;
            ...
            //Only attempt to restore the last cloned window if one was actually stored.
            bool hasStoredWindow = Settings.Default.RestoreLastWindowHwnd != 0 ||
                !string.IsNullOrEmpty(Settings.Default.RestoreLastWindowTitle) ||
                !string.IsNullOrEmpty(Settings.Default.RestoreLastWindowClass);
            if (Settings.Default.RestoreLastWindow && hasStoredWindow) {
                ...
                if (resultHandle != null) {
                    ...
                    Log.Write("Restoring last region: {0}", (object)options.Region ?? "(none)");
                }
                else {
        ...
        private static void ParseCommandLine(string[] args, Options options) {
            var cmdOptions = new NDesk.Options.OptionSet()
                .Add<int>("screen=", "Index of the {SCREEN} (monitor) to display on, starting from 0.", id => {
