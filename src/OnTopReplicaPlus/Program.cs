    ...
    static class Program {
        ...
        static void Main(string[] args) {
            ...
            using (_mainForm = new MainForm(options)) {
                ...
                //Persist the current opacity so it is restored on the next launch
                Settings.Default.Opacity = (byte)Math.Round(_mainForm.Opacity * 255.0);
        ...
        static void Application_Idle(object sender, EventArgs e) {
            ...
            //Automatic update checking is disabled in this fork: updates are obtained
            //manually from the GitHub releases page (see the About panel's button).
        ...
        static void CurrentDomain_UnhandledException(object sender, UnhandledExceptionEventArgs e) {
            ...
            using (var s = new FileStream(path, FileMode.Create)) {
                using (var sw = new StreamWriter(s)) {
                    sw.WriteLine("OnTopReplicaPlus Dump file");
                    sw.WriteLine("This file has been created because OnTopReplicaPlus crashed.");
                    sw.WriteLine("Please report it at https://github.com/K1tsune12/OnTopReplicaPlus/issues to help fix the bug that caused the crash.");
                    sw.WriteLine();
