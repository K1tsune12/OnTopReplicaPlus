...
namespace OnTopReplica {
    static class Log {
        ...
        static Log() {
            try {
                var filepath = Path.Combine(AppPaths.PrivateDataFolderPath, LogFileName);
                Writer = new StreamWriter(new FileStream(filepath, FileMode.Create));
                Writer.AutoFlush = true;
            }
            catch (Exception) {
                try {
                    var filepath = Path.Combine(AppPaths.PrivateDataFolderPath, string.Format(ConflictLogFileName, System.Diagnostics.Process.GetCurrentProcess().Id));
                    Writer = new StreamWriter(new FileStream(filepath, FileMode.Create));
                    Writer.AutoFlush = true;
                }
                catch (Exception) {
