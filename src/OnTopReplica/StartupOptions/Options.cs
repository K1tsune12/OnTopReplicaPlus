...
namespace OnTopReplica.StartupOptions {
    ...
    class Options {
        ...
        /// <summary>
        /// Index into <see cref="Screen.AllScreens"/> of the monitor the window should be
        /// placed on (0 = leave on the default/primary screen). Set via the /screen CLI flag.
        /// </summary>
        ...
        /// <summary>
        /// Offsets the form's location by the working-area origin of the target screen,
        /// so that a /position given relative to a screen ends up on that monitor.
        /// </summary>
        private void SetFormLocation(MainForm form, Screen screen) {
            Point start = StartLocation ?? form.Location;
            form.Location = new Point(
                start.X + screen.WorkingArea.Location.X,
                start.Y + screen.WorkingArea.Location.Y);
        }
        ...
        public void Apply(MainForm form) {
            ...
            //Move to the requested monitor, if any (and if the index is valid)
            if (ScreenIndex != 0 && ScreenIndex >= 0 && ScreenIndex < Screen.AllScreens.Length) {
